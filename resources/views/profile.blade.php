...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Notification icon - fetch events and show modal
    const btnNotification = document.getElementById('btn-notification');
    if (btnNotification) {
        btnNotification.addEventListener('click', async (e) => {
            e.stopPropagation();
            const modal = document.getElementById('custom-alert-modal');
            const listEl = document.getElementById('custom-alert-list');
            const okBtn = document.getElementById('custom-alert-ok');
            if (!modal || !listEl || !okBtn) return;
            ...
            try {
                ...
                const evs = (data.data || []).filter(e => e.status === 'upcoming');

                const getDaysUntil = (from, to) => {
                    const start = new Date(from); start.setHours(0,0,0,0);
                    const target = new Date(to); target.setHours(0,0,0,0);
                ...
                const targetDays = new Set([3, 5, 7]);
                const now = new Date(); now.setHours(0,0,0,0);
                const upcoming = evs
                    .filter(e => {
                        const start = new Date(e.date + 'T00:00:00');
                        if (start < now) return false;
                        return targetDays.has(getDaysUntil(now, start));
                    })
                    .map(e => ({ ...e, _days: getDaysUntil(now, e.date + 'T00:00:00') }))
                    .sort((a,b) => a._days - b._days || new Date(a.date) - new Date(b.date));

                if (upcoming.length === 0) {
                    listEl.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-muted);font-weight:600;">No upcoming reminders. Schedules 3, 5, or 7 work days away will appear here.</div>';
                } else {
                    listEl.innerHTML = upcoming.map(e => {
                        const title = (e.title || '').slice(0, 40) + ((e.title || '').length > 40 ? '…' : '');
                        const dateLabel = e.end_date && e.end_date !== e.date ? fmt(e.date) + ' – ' + fmt(e.end_date) : fmt(e.date);
                        const daysLabel = e._days === 0 ? 'Today' : e._days + ' work day' + (e._days === 1 ? '' : 's') + ' left';
                        return `<div class="reminder-item-real" data-event-id="${e.id}" style="display:flex;gap:12px;align-items:stretch;cursor:pointer;padding:12px;border-radius:12px;background:#fff;border:1px solid #e2e8f0;">
                            <div style="width:6px;border-radius:6px;background:${e.color||'#3b82f6'};flex-shrink:0;"></div>
                            <div style="min-width:0;flex:1;">
                                <div style="font-weight:800;color:${e.color||'#3b82f6'};font-size:13px;text-transform:uppercase;">${daysLabel}</div>
                                <div style="font-weight:700;color:var(--text-main);font-size:15px;margin:4px 0;">${title}</div>
                                <div style="color:#94a3b8;font-weight:600;font-size:13px;">${dateLabel}</div>
                            </div>
                        </div>`;
                    }).join('');
                    listEl.querySelectorAll('.reminder-item-real').forEach(item => {
                        item.addEventListener('click', () => {
                            const id = item.getAttribute('data-event-id');
                            if (id) window.location.href = '{{ route("dashboard") }}?view=' + encodeURIComponent(id);
                        });
                    });
                }
                ...
                newOk.addEventListener('click', () => modal.classList.remove('active'));
            } catch (err) {
                console.error(err);
                listEl.innerHTML = '<div style="padding:24px;text-align:center;color:#ef4444;">Failed to load notifications.</div>';
                modal.classList.add('active');
                const newOk = okBtn.cloneNode(true);
                okBtn.parentNode.replaceChild(newOk, okBtn);
                newOk.addEventListener('click', () => modal.classList.remove('active'));
