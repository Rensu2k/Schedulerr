...
function getFetchHeaders(method = 'GET') {
...
function openViewModal(eventId, dateContext) {
    ...
    if (badge) {
        badge.classList.remove('badge-upcoming', 'badge-completed', 'badge-cancelled');
        if (e.status === 'completed') {
...
function openNotificationModal(eventsList, options = {}) {
    ...
    if (upcoming.length === 0) {
        listHtml = '<div style="padding: 24px; text-align: center; color: var(--text-muted); font-weight: 600;">No upcoming reminders. Schedules 3, 5, or 7 work days away will appear here.</div>';
    } else {
        listHtml = upcoming.map((e) => {
            ...
            const daysLabel = e._workDaysLeft === 0 ? 'Today' : `${e._workDaysLeft} work day${e._workDaysLeft === 1 ? '' : 's'} left`;
            return `
    ...
    if (upcoming.length > 0) {
        const openView = onItemClick || ((id) => { if (typeof openViewModal === 'function') openViewModal(id); });
    ...
    newOk.addEventListener('click', () => alertModal.classList.remove('active'));
}
